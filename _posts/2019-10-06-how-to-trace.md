...
description: 本篇文章主要介绍本人在工作中是如何采用Spring AOP + Mybatis 拦截器 + Dubbo filter + ThreadLocal 结合APM分布式链路跟踪知识来实现分布式服务下用户行为及数据前后变更日志收集与审计分析。
---
## 前言
下面

## 需求实现
整篇文章会介绍使用到到技术点以及技术实现到难点，以及为什使用AOP实现链路数据收集而不是已有到APM分布式链路跟踪，欲知详情，请听我娓娓道来。
